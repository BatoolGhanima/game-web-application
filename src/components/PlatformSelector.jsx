...
const PlatformSelector = () => {
    ...
    const setSelectPlatform = store((state) => state.setSelectPlatform);
    const [isOpen, setIsOpen] = useState(false); // للتحكم في إظهار القائمة

    if (isError) return <p style={{ color: "red" }}>Error loading platforms!</p>;

    return (
        <>
            <button
                onClick={() => setIsOpen(!isOpen)} // يفتح القائمة عند الضغط
                    ...
                    <ul className="py-2 text-sm text-gray-700 dark:text-gray-200">
                        {data?.map((platform) => (
                            <li
                                ...
                                key={platform.id}
                                onClick={() => {
                                    setSelectPlatform(platform); // تحديد المنصة المختارة
